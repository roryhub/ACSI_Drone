...
class PID:
    ...
    def error_calc(self):
        self.current_global_error.time = rospy.Time.now().nsecs
        ...
        if len(self.global_error_hist) >= 10:
            ...
            Ts = (self.global_error_hist[0].time-self.global_error_hist[4].time)/(4*1e-9) #making sample time the average in the last five samples converted to seconds
